...
export default function ResultSummary({ result }: { result: Result }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
      {/* Banner */}
      <div className={`px-5 sm:px-8 py-6 ${accuracy >= 70 ? "bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50 dark:from-green-950 dark:via-emerald-950 dark:to-teal-950" : accuracy >= 50 ? "bg-gradient-to-r from-amber-50 via-yellow-50 to-orange-50 dark:from-amber-950 dark:via-yellow-950 dark:to-orange-950" : "bg-gradient-to-r from-red-50 via-rose-50 to-pink-50 dark:from-red-950 dark:via-rose-950 dark:to-pink-950"}`}>
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="relative shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">Accuracy</span>
            </div>
          </div>
          ...
          <div className="flex-1 text-center sm:text-left">
            <div className="flex items-center gap-2 mb-2 justify-center sm:justify-start">
              ...
              <span className="text-sm text-gray-500 dark:text-gray-400">{test?.exam?.name} · {test?.category?.name}</span>
            </div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-2 leading-tight">{test?.title}</h1>
            <p className={`text-lg font-bold ${grade.color}`}>{grade.label}</p>
            {test?.passing_marks && (
              <div className={`inline-flex items-center gap-1.5 mt-2 text-xs font-semibold px-3 py-1 rounded-full ${passed ? "bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300" : "bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-300"}`}>
          ...
          <div className="text-center shrink-0">
            <div className="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white">{result.score.toFixed(result.score % 1 === 0 ? 0 : 1)}</div>
            <div className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">out of {result.max_score}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-gray-100 dark:divide-gray-800 border-y border-gray-100 dark:border-gray-800">
        {[
          { icon: <CheckCircle className="w-5 h-5 text-green-600" />, label: "Correct", value: result.correct_count, sub: `${Math.round((result.correct_count / result.total_questions) * 100)}% of total`, bg: "bg-green-50 dark:bg-green-950" },
          { icon: <XCircle className="w-5 h-5 text-red-500" />, label: "Wrong", value: result.wrong_count, sub: "marks deducted", bg: "bg-red-50 dark:bg-red-950" },
          { icon: <MinusCircle className="w-5 h-5 text-gray-400" />, label: "Skipped", value: result.skipped_count, sub: "no penalty", bg: "bg-gray-50 dark:bg-gray-800" },
          { icon: <Clock className="w-5 h-5 text-blue-600" />, label: "Time Taken", value: formatTime(result.time_taken_seconds), sub: `of ${test?.timer_minutes}m allowed`, bg: "bg-blue-50 dark:bg-blue-950" },
      ...
      <div className="px-5 sm:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-5 text-sm flex-wrap justify-center sm:justify-start dark:text-gray-300">
          {result.rank && (
        ...
        <div className="flex items-center gap-2 flex-wrap justify-center">
          <Link href={`/test/${result.test_id}?reattempt=1`}
            className="flex items-center gap-1.5 text-sm font-semibold border border-gray-200 dark:border-gray-700 px-4 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-gray-700 dark:text-gray-300">
