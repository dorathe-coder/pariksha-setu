...
import { Upload, FileText, ClipboardPaste, Plus, CheckCircle, AlertCircle, X, Loader2, ChevronDown, ChevronUp } from "lucide-react";
  ...
  blocks.forEach((block, idx) => {
    try {
      ...
      lines.slice(1).forEach((line) => {
        ...
        const aM = line.match(/^[Aa][.)\s]+(.+)/);
        const bM = line.match(/^[Bb][.)\s]+(.+)/);
        const cM = line.match(/^[Cc][.)\s]+(.+)/);
        const dM = line.match(/^[Dd][.)\s]+(.+)/);
        ...
        else if (aM) optA = aM[1].trim();
        else if (bM) optB = bM[1].trim();
        else if (cM) optC = cM[1].trim();
        else if (dM) optD = dM[1].trim();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <h3 className="font-semibold text-gray-900 mb-4">Step 1: Select Test and Settings</h3>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
          <button onClick={() => setMode("paste")}
            className={"flex flex-col items-start gap-2 p-4 rounded-xl border-2 transition-all text-left " + (mode === "paste" ? "border-blue-900 bg-blue-50" : "border-gray-100 hover:border-gray-200")}>
            <ClipboardPaste className={"w-5 h-5 " + (mode === "paste" ? "text-blue-900" : "text-gray-500")} />
            <div>
              <div className={"text-sm font-semibold " + (mode === "paste" ? "text-blue-900" : "text-gray-800")}>Copy-Paste Questions</div>
              <div className="text-xs text-gray-500">Paste questions copied from PDF or text</div>
            </div>
            <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">Fastest</span>
          </button>

          <button onClick={() => setMode("csv")}
            className={"flex flex-col items-start gap-2 p-4 rounded-xl border-2 transition-all text-left " + (mode === "csv" ? "border-blue-900 bg-blue-50" : "border-gray-100 hover:border-gray-200")}>
            <FileText className={"w-5 h-5 " + (mode === "csv" ? "text-blue-900" : "text-gray-500")} />
            <div>
              <div className={"text-sm font-semibold " + (mode === "csv" ? "text-blue-900" : "text-gray-800")}>Upload CSV</div>
              <div className="text-xs text-gray-500">Upload .csv file with question data</div>
            </div>
          </button>

          <button onClick={() => setMode("manual")}
            className={"flex flex-col items-start gap-2 p-4 rounded-xl border-2 transition-all text-left " + (mode === "manual" ? "border-blue-900 bg-blue-50" : "border-gray-100 hover:border-gray-200")}>
            <Plus className={"w-5 h-5 " + (mode === "manual" ? "text-blue-900" : "text-gray-500")} />
            <div>
              <div className={"text-sm font-semibold " + (mode === "manual" ? "text-blue-900" : "text-gray-800")}>Add Manually</div>
              <div className="text-xs text-gray-500">Add one question at a time</div>
            </div>
          </button>
        </div>
        ...
        {mode === "paste" && (
          <div>
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-4 text-sm text-amber-800">
              <strong>Format Guide:</strong> Paste questions from any PDF. System auto-detects question numbers, A/B/C/D options, and Answer lines.
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <button onClick={handleParse} className="bg-blue-900 hover:bg-blue-950 text-white font-semibold px-6 py-2 rounded-xl text-sm transition-colors">
                Parse Questions
              </button>
            </div>
          </div>
        ...
        {mode === "manual" && (
          <div className="text-center py-6">
            <button onClick={() => setParsedQuestions((prev) => [...prev, { _id: Date.now(), question_text: "", option_a: "", option_b: "", option_c: "", option_d: "", correct_option: "A", difficulty: "medium" }])}
      ...
      {parseErrors.length > 0 && (
        <div className="bg-red-50 border border-red-100 rounded-2xl p-4">
          <p className="text-sm font-semibold text-red-700 mb-2 flex items-center gap-1.5">
            <AlertCircle className="w-4 h-4" /> {parseErrors.length} parsing errors (skipped)
          </p>
            ...
            {parsedQuestions.map((q, idx) => (
              <div key={q._id} className="border border-gray-100 rounded-xl overflow-hidden">
                <div className="flex items-start gap-3 p-3.5 cursor-pointer hover:bg-gray-50 transition-colors"
                  ...
                  <p className="text-sm text-gray-800 flex-1 line-clamp-1">{q.question_text || <span className="text-gray-400 italic">Empty</span>}</p>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={"badge " + (q.difficulty === "easy" ? "bg-green-100 text-green-700" : q.difficulty === "hard" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700")}>{q.difficulty}</span>
                    <span className="badge bg-gray-100 text-gray-700">Ans: {q.correct_option}</span>
                    <button onClick={(e) => { e.stopPropagation(); removeQuestion(q._id); }} className="text-gray-300 hover:text-red-500 transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                ...
                {expandedIdx === idx && (
                  <div className="px-3.5 pb-3.5 border-t border-gray-50 pt-3 space-y-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {["a", "b", "c", "d"].map((opt) => (
                        <div key={opt}>
                          <label className={"text-xs font-medium mb-1 block " + (q.correct_option === opt.toUpperCase() ? "text-green-700" : "text-gray-500")}>
                          ...
                          <input value={(q as any)["option_" + opt] || ""} onChange={(e) => updateQuestion(q._id, "option_" + opt, e.target.value)}
                            className={"w-full border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-900 " + (q.correct_option === opt.toUpperCase() ? "border-green-300 bg-green-50" : "border-gray-200")} />
                        </div>
                      ))}
                    </div>
